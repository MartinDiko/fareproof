...
## Agency price determines booking readiness
- **Confirmed:** 2026-07-10 — Martin reported that "BookWithMatrix accepted" provided no useful booking link and clarified that the agency price produced through BookWithMatrix matters because Matrix prices frequently do not survive on real booking sites.
- **Rule:** Treat BookWithMatrix acceptance only as a pending handoff. Visit each explicitly supported agency link it produces, keep progressively loading pages pending, persist the agency's visible price and incomplete or failed evidence, compare that price with Matrix and BookWithMatrix, and use the agency price as the policy and alert source of truth even when it differs. Enable the booking action only after that agency reproduces route, date, flight identity, long-leg cabin, and an original-currency price within the policy; otherwise provide the agency result only as a manual-review link.
- **Why:** A parsed handoff and a Matrix quote do not establish a purchasable fare. The decision must come from the current agency result where the user can actually continue booking.
- **Reference:** `validateRetailerObservation`, `handleRetailerPage`, `buildFareEvidenceView`, and `tests/e2e/extension.spec.ts`.

## Every run is visible and prices normalize to CAD
- **Confirmed:** 2026-07-10 — Martin reported that one Matrix outage appeared as errors across the other searches, requested a better extension UI with every past run listed, required USD prices to be converted to CAD, and requested detailed solution architecture documentation.
- **Rule:** Record one durable history entry for every manual or scheduled run started after version 0.2.5, with trigger, timing, aggregate and policy outcomes, source price, original currency, CAD price, FX metadata, agency, and booking URL. Represent a bounded Matrix site outage as one `matrix-unavailable` run and `blocked` policy statuses rather than FareProof errors. Convert USD agency and BookWithMatrix values with the validated Bank of Canada daily USD/CAD rate before applying CAD policy limits, while preserving the original USD quote and rate. Keep `docs/architecture.md` aligned with implemented boundaries and limitations.
- **Why:** Current policy cards are snapshots, not an execution ledger; repeating one upstream outage as several errors obscures the actual failure. CAD normalization makes policy comparisons meaningful without discarding the price the agency actually displayed.
- **Reference:** `RunHistory`, `runHistoryEntrySchema`, `exchangeRates.ts`, `migratePolicyStatus`, and `docs/architecture.md`.

## "Matrix unavailable" on every scheduled run is an automation failure, not a site outage
- **Confirmed:** 2026-07-11 — Martin asked why he gets a lot of Matrix-unavailable results; every history row was a scheduled run failing identically.
- **Rule:** When every scheduled run reports "ITA Matrix did not return fare data after two attempts," treat it as a systemic FareProof automation failure (ITA DOM/selector drift in `submitMatrixSearch`/`extractMatrixCalendar`, or background-tab throttling of the non-interactive owned tab), not a bounded upstream outage. Confirm by running Check now interactively (visible, focused tab) with DevTools open and watching where the form fills or the calendar stalls; if interactive works but scheduled does not, the cause is background-tab throttling, and if a specific control is never found, re-map that selector. A single genuine `matrix-unavailable` is expected occasionally; a continuous streak across all policies is a defect to fix, not to report.
- **Why:** The two-attempt failure fires when the calendar stage times out or the form throws, so an unbroken streak means zero successful searches — the fragile ITA scraping is the product's weak point and the practical motivation for an independent fare source.
- **Reference:** `submitMatrixSearch` in `packages/extension/src/content/ita/formAutomation.ts`, `extractMatrixCalendar` in `packages/extension/src/content/ita/extraction.ts`, and `retryMatrixCalendar`/`failMatrixRun` in `packages/extension/src/background/serviceWorker.ts`.

## Flight-price API discovery is an opt-in extension source, never a web-app backend
- **Confirmed:** 2026-07-11 — Martin asked to use free flight-site APIs to search fares and build that integration with the web app; he was unavailable, so the pattern-faithful choice was made autonomously.
- **Rule:** There is no free, keyless, CORS-open, bookable-fare API (Google's ITA QPX was retired in 2018), so any provider needs a secret token and server-side calls. Add flight-price API search as an optional, user-configured source in the extension: the user supplies their own provider token stored in local storage, the provider host is an explicitly requested `optional_host_permission`, and settings state the external-data disclosure — mirroring the opt-in ntfy pattern. API results are indicative discovery candidates (`verificationStage: 'search-result-reproduced'`, cabin/availability unconfirmed) that still require the existing retailer-validation stage before a booking-ready alert; never present them as validated fares. The static Pages app keeps no backend and no key — it receives API candidates only through the versioned export bundle. A serverless proxy that lets the web app search live remains a separate, explicit decision that would change the local-only/no-backend rule.
- **Why:** This adds an independent fare source that does not depend on fragile Matrix scraping, without breaking the local-first privacy boundary or leaking a key in static client JS.
- **Status:** Travelpayouts/Aviasales adapter is fixture-backed and unit-tested in core; the extension wiring (settings, background fetch, opt-in alarm, export) typechecks, lints, and builds but is not yet verified live against the API with a real token.
- **Reference:** `packages/core/src/travelpayouts.ts`, `packages/core/src/travelpayouts.test.ts`, `packages/extension/src/background/apiSearch.ts`, `apiSearchSettingsSchema`, and the `https://api.travelpayouts.com/*` optional host permission.