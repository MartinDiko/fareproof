...
- Scheduler recovery validates that a persisted run still owns a real Chrome tab and has not exceeded its stage timeout. Extension startup, automatic alarms, and **Check now** clear stale state; Check now displays a reason if another healthy run or a scheduler error prevents startup.
- Visible Matrix flexible-date form submission through semantic controls, with the browser-generated base64 URL retained for diagnostics; calendar and flight-result capture; candidate selection by lowest price and then shortest duration; interception and Zod validation of Matrix's own **Copy itinerary as JSON** output.
